...
| 応答 | 扱い |
| --- | --- |
| タイムアウト / 接続失敗 / 5xx | リトライする |
| **404** | リトライする |
| 400 / 401 / 403 | リトライしない。終了コード `2` |
| `{"ok": false}`（トークン不一致など） | リトライしない。終了コード `2` |

404 をリトライ対象にしているのは、**Apps Script が正常なデプロイに対しても断続的に404を返す**
ため（実測: 同じURLで 17:00 成功 → 17:30 が404 → 18:00 成功）。3回とも404だった場合は、
本当にURLが古い可能性もあるため、`GOOGLE_SCRIPT_URL` の確認を促すメッセージを添える。

終了コード `2` は「設定を直さないと解決しない」という意味で、`systemd` 側もこの終了コードでは
再試行しない（`RestartPreventExitStatus=2`）。
